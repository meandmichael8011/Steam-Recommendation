...
st.image('photo.jpg')
urgame = st.text_input("Enter your link:")
if st.button("Start"):
    ...
    for x in finallist:
        ...
        try:
            ...
            print(
                f"{nick} is not on Steam and/or not sold in Russia - it should be played on a different platform. Its tags are:")
        print(get_tags(x))
    ...
    for x in t1:
        if st.button(x):
            kokc = []
            for qe in df["Mutual Tags"]:
                if x in qe:
                    kokc.append(qe)
            indices = []
            for joe in kokc:
                indices.append(df[df["Mutual Tags"] == joe].index.values.astype(int)[0])
            df.iloc[indices]
